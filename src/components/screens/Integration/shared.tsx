...
export function Workspace({ user, title, children }: { user: SessionUser; title: string; children: ReactNode }) {
  const navigation = user.role === "admin" ? [
    { label: "Coordenadores", href: "/admin#coordinators", icon: "users" as const },
    { label: "Criar coordenador", href: "/admin/coordenadores/novo", icon: "users" as const },
  ] : user.role === "coordenador" ? [
    { label: "Turmas", href: "/coordenador/turmas", icon: "book" as const },
    { label: "Alunos", href: "/coordenador#students", icon: "users" as const },
  ] : user.role === "professor" ? [
