...
export function useCreateStudent(user: SessionUser) {
  ...
  function setField(field: keyof StudentForm, value: string) {
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!parsed.success) {
      ...
      for (const issue of parsed.error.issues) nextErrors[issue.path[0] as keyof StudentForm] ??= issue.message;
