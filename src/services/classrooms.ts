...
export function removeClassroomTeacher(user: SessionUser, roomId: string, teacherId: string, rooms: Classroom[]): Classroom[] {
  const room = rooms.find((item) => item.id === roomId);
  if (!room || user.role !== "coordenador" || !schools.some((school) => school.id === room.schoolId && school.coordinatorId === user.id)) {
...
export function addClassroomTeachers(user: SessionUser, roomId: string, teacherIds: string[], rooms: Classroom[]): Classroom[] {
  const room = rooms.find((item) => item.id === roomId);
  if (!room || user.role !== "coordenador" || !schools.some((school) => school.id === room.schoolId && school.coordinatorId === user.id)) {
