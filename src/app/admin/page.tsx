...
export default function AdminHome() {
  ...
  return (
    <DashboardShell user={user} title="Uma visão de toda a rede." description="Organize as escolas e conecte cada uma à sua liderança pedagógica." navigation={[{ label: "Escolas", href: "/admin/escolas/nova", icon: "school" }, { label: "Coordenadores", href: "/admin/coordenadores/novo", icon: "users" }, { label: "Vínculos pendentes", href: "#pending", icon: "check" }]}>
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[1.6fr_1fr]">
        <DashboardPanel id="schools" title="Escolas da rede" description="Cada escola tem seu espaço e um coordenador responsável.">
          ...
          <div className="space-y-4">{schools.map((school) => {
            ...
            return <Link key={school.id} href={`/admin/escolas/${school.id}`} className="block rounded-xl border border-[var(--line)] p-4 transition hover:border-[var(--blue)] hover:shadow-md focus-visible:outline-2 focus-visible:outline-[var(--blue)]">
              <div className="flex flex-wrap items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#e8f5f8] text-sm font-semibold text-[var(--blue)]">{school.initials}</span><div className="flex-1"><h3 className="text-sm font-semibold">{school.name}</h3><p className="mt-1 text-xs text-[var(--muted)]">{school.city}{school.state ? ", " + school.state : ""}</p></div><span className={`rounded-full px-3 py-1 text-[10px] font-medium ${coordinator ? "bg-[#edf8f2] text-[#30714b]" : "bg-[#fff5d5] text-[#846900]"}`}>{coordinator ? "Coordenador vinculado" : "Vínculo pendente"}</span></div>
              {school.cnpj && <p className="mt-3 text-xs text-[var(--muted)]">CNPJ: {school.cnpj}</p>}{school.street && <p className="mt-1 text-xs text-[var(--muted)]">{school.street}</p>}
        ...
        <div className="space-y-6">
          <DashboardPanel id="coordinators" title="Coordenadores" description="Perfis criados pelo administrador.">
            {coordinators.map((person) => <div key={person.id} className="rounded-xl bg-[#f5f9fa] p-4"><p className="text-sm font-semibold">{person.name}</p><p className="mt-1 break-all text-xs text-[var(--muted)]">{person.registration ? "Matrícula: " + person.registration : person.email}</p><p className="mt-3 text-xs font-medium text-[var(--blue)]">{schools.filter((school) => school.coordinatorId === person.id).map((school) => school.name).join(", ") || "Sem escola vinculada"}</p></div>)}
          </DashboardPanel>
